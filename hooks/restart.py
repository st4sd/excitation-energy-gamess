...
def get_geometry_from_dat_file(filein):
    ...
    with open(filein, "r") as fin:
        try:
            ...
            log.error("ERROR - value error evaluating max of the line number matching {} in get vec data".format(search_start_term))
            return False
        fin.seek(0)
        ...
        try:
            ...
            log.error("ERROR - value error evaluating max of the line number matching {} in get vec data".format(search_end_term))
            return False
        fin.seek(0)
    ...
    with open(filename, "r") as fin:
        ...
        if end <= start:
            log.error("ERROR - $VEC data - start and stop lines are the same or stop is less than start. \
                    Check {}\nstart regex is {}\nstop regex is {}\n.".format(filename, search_start_term, search_end_term))
            return False
...
def determine_gamess_exit_reason(gamess_out_file, component_type):
    ...
    if run_type == "ENERGY":
        ...
        reasons = "ddick sent kill: {} terminated on error: {} stationary point not located: {}".format(ddikick_failed, term_abnorm, stat_point_not_found)
        log.error("GAMESS has failed for the following reasons {} but can be restarted".format(reasons))
    else:
        fails = False
   ...
   Any files created/renamed/moved must be in the workingDir or subdirs
    ...
    log.info("Restart:\nworking dir: {}\nNumber of restarts: {}\n"
            "Component name: {}\nLog: {}\nExit reason {}\nExit code {}".format(workingDirectory,
                restarts, componentName, log, exitReason, exitCode))
   
    ##############
    restart_threshold = 4 # Will not restart more than this number of times for a single component
    ##############
    ...
    subdirs = [d for d in os.listdir(workingDirectory) if os.path.isdir(os.path.join(workingDirectory, d)) and re.match(r"Run\d*", d)]
    ...
    if retgopt or retenr:
        ...
        if len(gamess_out_file) == 0:
            log.error("Restart cannot find the appropriate gamess out file (out.stdout) file in {}".format(workingDirectory))
            #raise RuntimeError("ERROR - cannot find GAMESS punch file *.dat")
            return False
        ...
        log.info("GAMESS Input: {} GAMESS punch file {} GAMESS out file {}".format(gamess_input,gamess_dat_punch_file, gamess_out_file))
        ...
        if restart_needed:
            ...
            write_file(gamess_input, geometry=geometry, template_data=template_data, molecular_orbitals=molecular_orbitals, norbs=norbs, restart=restarts)

            if delete_old is True:
                all_fs = [os.path.join(workingDirectory,f) for f in os.listdir(workingDirectory) if os.path.isfile(os.path.join(workingDirectory,f))]
