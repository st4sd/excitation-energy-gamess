...
if __name__ == "__main__":
    ...
    header = ['label', 'completed', 'total-energy', 'homo', 'lumo', 'gap',
              'electric-moments', 'total-time', 'total-time-per-core', 'excitation-energy', 'osc-str-max']
    ...
    for (f_out,f2) in zip(args,options.labels.split(',')):
        ...
        if data_ON == ['OK']:
            ...
            # ===== TD-DFT (osc.str_max) =====
            if TDDFT == 'Y':
                ind = txt_O.find(' 0 ->             HARTREE          EV         X       Y       Z     STRENGTH')
                txt5 = txt_O[ind + len(' 0 ->             HARTREE          EV         X       Y       Z     STRENGTH\n'):]
                ind = txt5.find('\n TRANSITION        EXCITATION            TRANSITION DIPOLE, A.U.     OSCILLATOR')
                txt5 = txt5[:ind]
                tddft_sum = txt5.split('\n')
                ex_os_l = []
                for tit in tddft_sum[1:-1]:
                    ex_os_l.append([tit.split()[3], tit.split()[7]])
                ex_os_l.sort(key = lambda x: x[1], reverse=True)
                data_tddft = ex_os_l[0]
                data = data + [data_tddft[0]] + [data_tddft[1]]
            else:
                data = data + ['0'] + ['0']
